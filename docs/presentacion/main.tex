...
{ctemes \and @pepdiz \and Milo \and salvari}
...
\begin{frame}{Montaje I}
  \includegraphics [width=0.8\textwidth]{johny5.jpg}
...
\begin{frame}[fragile]{Servo}
  \begin{onlyenv}<1>
    \includegraphics [width=0.6\textwidth]{sg90servo.jpg}
  \end{onlyenv}
\begin{onlyenv}<2>
  \begin{lstlisting}
#include <Servo.h>     
...
\begin{frame}{Processing}
  ...
  \end{onlyenv}
\note[item]<1>{Qué es processing?}
\note[item]<1>{Processing es java}
\note[item]<1>{Educativo}
\note[item]<1>{Interpretado no compilado}
\note[item]<1>{Artístico}
\note[item]<1>{Ben Fry and Casey Reas (MIT)}
\end{frame}
...
%----------------------------------------------------------------------
\begin{frame}{Pantalla de Processing}{Trigonometría}
  \begin{columns}
    \column{0.6\textwidth}
    \includegraphics [width=0.9\textwidth]{sohcahtoa.jpg}
    \column{0.4\textwidth}
  \end{columns}
\end{frame}
